...
Specifying the Wwise SDK
^^^^^^^^^^^^^^^^^^^^^^^^

In order to build the Steam Audio Wwise plugin, you will need to ensure that an appropriate Wwise SDK is installed. You can install the Wwise SDK from the `Wwise Launcher <https://www.audiokinetic.com/en/public-library/Launcher_2025.3.3.5754/?source=InstallGuide&id=modyfing_wwise>`_.

.. note::
    Wwise SDK versions 2023.1.0 or later are supported. Ideally, you should use a Wwise SDK version that matches the version of the Wwise authoring tool you are using, and the version of the Wwise SDK integrated into your game.
    
    Make sure that you install all SDK deployment platforms that you intend to build the Steam Audio Wwise plugin for.

Before proceeding, make sure that the ``WWISEROOT`` environment variable is set to point to the root directory of the Wwise installation you want to build the Steam Audio Wwise plugin against. This can also be done using the `Wwise Launcher <https://www.audiokinetic.com/en/public-library/Launcher_2025.3.3.5754/?source=InstallGuide&id=modyfing_wwise>`_.

Building using the build script
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^

.. note::
    Before building the plugin, ensure that you have built the Steam Audio SDK by following the instructions in the Steam Audio SDK documentation. This will ensure that the necessary Steam Audio SDK headers and libraries are available for the Wwise integration.
...
As an example, from the ``wwise`` directory of the repository::
