...
# Steam Audio Unity Plugin <small><small>2.0-beta.16</small></small>
...
Exporting a scene as described above means that if the geometry moves in any way at run-time, the acoustics are _not_
updated accordingly. This is not always desirable. If your game or app only uses occlusion, and not reflections or
reverb, then Steam Audio can be configured to use Unity's built-in ray tracer to handle dynamic geometry. To do this:
